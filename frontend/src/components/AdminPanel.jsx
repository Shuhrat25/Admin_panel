...
// Расширенные тестовые данные для демонстрации пагинации
const generateMockUsers = () => {
  const users = [
    { id: 1, name: 'Alexander', email: 'alex@gm.c', lastLogin: '2024-04-23T13:35:53', status: 'active' },
    { id: 2, name: 'Test User', email: 'test0@test.com', lastLogin: '2024-04-23T13:51:35', status: 'blocked' },
    { id: 3, name: 'Nina Simene', email: 'nina@example.com', lastLogin: '2024-04-24T10:15:00', status: 'unverified' },
    { id: 4, name: 'Jim Morrison', email: 'dmtimer9@dealyaari.com', lastLogin: '2024-04-25T09:00:00', status: 'active' },
  ];
  // Генерируем еще 30 пользователей для проверки страниц
  for (let i = 5; i <= 35; i++) {
    users.push({
      id: i,
      name: `User ${i}`,
      email: `user${i}@example.com`,
      lastLogin: `2024-04-${Math.floor(Math.random() * 20 + 10).toString().padStart(2, '0')}T10:00:00`,
      status: ['active', 'blocked', 'unverified'][Math.floor(Math.random() * 3)]
    });
  }
  return users;
};
...
function AdminPanel() {
  ...
  // --- Логика обработки данных (Поиск -> Сортировка -> Пагинация) ---
  const processedUsers = useMemo(() => {
    ...
    if (sortConfig.key) {
      result.sort((a, b) => {
        ...
        // Используем localeCompare с numeric: true для правильной сортировки чисел в строках
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      // Выделяем только тех, кто виден на текущей странице
  ...
  // Функция для рендера иконки сортировки
      ...
      <Navbar bg={isDarkMode ? 'dark' : 'white'} variant={isDarkMode ? 'dark' : 'light'} className="shadow-sm mb-4 px-4 border-bottom">
        <Container fluid>
          ...
          <Nav className="ms-auto align-items-center">
            ...
            <Button variant={isDarkMode ? 'outline-light' : 'outline-dark'} size="sm">
        ...
        <Card className={`shadow-sm border-0 ${isDarkMode ? 'bg-dark border border-secondary' : ''}`}>
          <Card.Body className="p-0">
            ...
            <div className={`row g-3 align-items-center p-3 border-bottom m-0 ${isDarkMode ? 'bg-secondary bg-opacity-25' : 'bg-light'}`}>
              ...
              {/* 3. Правая сторона: Количество записей */}
                ...
                <tr>
                  <th style={{ width: '50px', textAlign: 'center', verticalAlign: 'middle' }}>
                    <Form.Check
                      ...
                      ref={input => {
                        if (input) {
                          // Indeterminate state если выбрана часть
                          input.indeterminate = selectedIds.length > 0 && selectedIds.length < paginatedUsers.length;
                        }
                      }}
                    />
                  </th>
